...
function crearFruta(form1,type){
    let fruta = {};
    if(type == 1){   
        fruta = {
            ...
            Disponibility: form1.get("inlineRadioOptionsreg"),
            fecha:agregarFecha()
            };
        } 
    else if(type == 2){        
        fruta = {
            ...
            Disponibility:form1.get("inlineRadioOptionsEdit"),
            fecha:agregarFecha()            
        };
    }    
	console.log(form1.get("inlineRadioOptionsreg"))   
    return fruta
}
...
function registrarFruta(Form1){
    let fruta = crearFruta(Form1,1)
    if(fruta.id != ""){
        lista_frutas.unshift(fruta)
        insertarFruta(fruta)
...
function insertarFruta(fruta){
    ...
    formFrutareg.reset();   
        ...
        newlistaCell.textContent = fruta.Disponibility

        newlistaCell = newlistaRow.insertCell(3);
        newlistaCell.textContent = fruta.Unitcost
...
formFrutareg.addEventListener("submit", function(event){
...
// Función eliminar un elemento de la lista
...
function eliminarFruta(id){
 ...
 // Permite remplazar un elemento de la lista

function remplazarFruta(form3){
    let fruta = crearFruta(form3,2);
...
let autocompletarForm = (id)=>{
    ...
    if(infofruta.Disponibility=="Disponible"){
        document.getElementById("inlineRadio1Edit").checked=true
    }
    else {
        document.getElementById("inlineRadio2Edit").checked=true
    }
}
...
formFrutaedit.addEventListener("submit", function(event){
    event.preventDefault();
    swal({
        title: "Estás seguro?",        
        icon: "warning",
...
let busqueda = (keyWord)=>{
    let isElement = false;
    for(let fruta in lista_frutas){
        if(lista_frutas[fruta].id == keyWord || lista_frutas[fruta].namefruit  == keyWord ){
            limpiarTabla()
            isElement = true
